"""DEPLOYMENT-MODE distilled student: train on ALL 7 cameras (no holdout).

Teacher = the k=2 trio ensemble space (mean of s7/s8/s9 embeddings from
`_vitb_dst_emb_v4.npz`), which covers all 997 embedded tracklets INCLUDING the
oblique camera 66.130. Links are mined across ALL camera pairs, so the
oblique-vs-dorsal alignment is finally trained explicitly instead of relying on
backbone generalisation. Target metric = FULL TRANSDUCTIVE (current best 0.516).

Same student design as vitb_unsup_distill.py: fixed intra-camera clusters (teacher
space) + pairwise (non-transitive) mutual-kNN link loss + intra CE, from-scratch,
early stop. Identities appearing ONLY in 66.130 (~17) lack teacher embeddings and
are excluded (they carry no cross-camera signal anyway).

Train:  python vitb_unsup_deploy.py --seed 10 --ckpt _vitb_dep_s10_ckpt.pt --wall 240 --target 1000
"""
...
from cowreid import CameraTopology
...
def main():
    ...
    ap.add_argument("--ckpt", default="_vitb_dep_s10_ckpt.pt")
    ...
    # ---------- teacher space: k=2 trio ensemble over ALL cameras ---------- #
    d = np.load(args.teacher_npz, allow_pickle=True)
    ids = list(d["ids"]); pos = {t: i for i, t in enumerate(ids)}
    mats = [d[s] for s in sorted(d.files) if s != "ids" and (any(k in s for k in K2) or s == "t0")]
    Xt = np.mean(mats, axis=0)
    Xt = Xt / (np.linalg.norm(Xt, axis=1, keepdims=True) + 1e-12)

    g_tids = [t for t in ids]                          # all embedded tracklets, 7 cams
    cams_list = [cam_of(t) for t in g_tids]
    cams = sorted(set(cams_list))
    print(f"[deploy] {len(g_tids)} tracklets across {len(cams)} cameras", flush=True)
    ...
    for c, ts in sorted(by_cam.items()):
        ...
        n_cls[c] = k; off += k
    n_proxy = off
    print(f"[teacher] {n_proxy} fixed intra-camera proxies (7 cams)", flush=True)
    ...
    n_obl = sum(1 for a, b in pairs if "66.130" in (cam_of(g_tids[a]), cam_of(g_tids[b])))
    print(f"[teacher] {len(pairs)} mutual-NN k={args.link_k} links, "
          f"precision={correct / max(len(pairs), 1):.3f}, {n_obl} involve 66.130",
          flush=True)
