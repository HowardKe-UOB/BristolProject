...
## (1) Intra-camera clustering algorithm  [vitb_unsup_cap.py:146-156; cowreid/cluster.py:46-98]
Per camera, tracklet-mean frozen features are clustered by ClusterAssigner(0.7, 10): L2-normalize features; cosine sim matrix; build mutual-kNN edges (i,j) iff j is in i's top-k=10 AND i is in j's top-k=10 AND sim(i,j) >= 0.7; sort edges by descending similarity; add them to a CONSTRAINED union-find that refuses any union which would place a cannot-link pair in the same component (checks all cross pairs of the two components' member sets, cowreid/pair_miner.py:347-361). min_cluster_size=1 so no cluster is discarded. Cluster count per camera (or 1 if zero) sets that camera's classifier size n_cls[c].
...
## Cannot-link set construction  [cowreid/cluster.py:21-43; vitb_unsup_cap.py:122-125]
...
## Camera topology source (GT oracle)  [cowreid/topology.py:47-63; vitb_unsup_cap.py:124-125]
...
## (2) CA-Jaccard distance (inter-camera clustering metric)  [cowreid/cajaccard.py:20-65]
...
## (2) DBSCAN on CA-Jaccard  [cowreid/cajaccard.py:68-90; vitb_unsup_cap.py:114,184-185]
...
## (3) Crop bags for OT mining  [cowreid/crossview_ot.py:21-44; vitb_unsup_cap.py:105,134-136]
...
## (3) Dustbin-OT matching (Sinkhorn with reject option)  [cowreid/crossview_ot.py:62-81; cowreid/sinkhorn.py:12-89]
...
## (3) Vote aggregation into tracklet must-links  [cowreid/crossview_ot.py:62-86; vitb_unsup_cap.py:196-197]
...
## (5) Intra-camera multi-branch CE branch (even steps)  [vitb_unsup_cap.py:203-215; cowreid/iics.py:54-75]
...
## (5) Losses NOT present in the CAP script  [vitb_unsup_cap.py:203-228; train_phase2.py:33-45; cowreid/losses.py:28-199; train_finetune_iics.py:260-277]
...
## (8) Backbone and unfrozen layers  [vitb_unsup_cap.py:158-159; vitb_unsup.py:36; cowreid/encoder.py:37-49]
...
## (8) Head architecture / embedding definition  [train_finetune_iics.py:35-54; cowreid/iics.py:34-79; cowreid/encoder.py:55-70]
...
## Where GT enters Stage 1  [vitb_unsup_cap.py:124-126,196-197; cowreid/topology.py:47-63]
...
- n_blocks unfrozen (ViT-B last blocks + final norm) = 4   (vitb_unsup_cap.py:113; cowreid/encoder.py:37-48)
...
- DBSCAN min_samples = 2 (noise points become singleton clusters)   (cowreid/cajaccard.py:69,84-89)
- CA-Jaccard k1 (k-reciprocal) = 20 (local expansion uses round(k1/2)=10 with 2/3 overlap rule)   (cowreid/cajaccard.py:27,41-42)
- CA-Jaccard k2 (local query expansion) = 6   (cowreid/cajaccard.py:27,53-54)
- CA-Jaccard inter-camera neighbours added per sample = max(1, k1//2) = 10   (cowreid/cajaccard.py:49)
- CA-Jaccard lambda (original-distance mixing) = 0.0 (pure Jaccard)   (cowreid/cajaccard.py:28,62-63)
- Intra-camera ClusterAssigner sim_threshold = 0.7   (vitb_unsup_cap.py:152; cowreid/cluster.py:49)
- Intra-camera ClusterAssigner k (mutual-kNN) = 10 (min_cluster_size=1)   (vitb_unsup_cap.py:152; cowreid/cluster.py:49-50)
- Topology overlap threshold = 0.02 (cannot-link + crop-bag eligibility)   (vitb_unsup_cap.py:125; cowreid/crossview_ot.py:21)
- Tracklet building max_gap_s = 2   (vitb_unsup_cap.py:122)
- Sinkhorn entropic eps (OT) = 0.1 (n_iters=200, tol=1e-6, log-domain)   (cowreid/crossview_ot.py:62; cowreid/sinkhorn.py:13)
- Dustbin cost = 0.5-quantile (median) of the bag's real cost entries; corner cell = 0   (cowreid/sinkhorn.py:50,68-73)
...
- Cosine-classifier logit scale = 16.0 (bias-free, L2-normalized weights and embeddings)   (cowreid/iics.py:73-75)
...
- Champion odd-step objective weights (train_finetune_iics/vitb_unsup, for contrast) = w_contrastive=1.0 (temp 0.07, hard-negative weight 2.0), w_cluster=1.0 (ClusterNCE temp 0.05, memory momentum 0.2), w_cannotlink=0.5 (hinge margin 0.0)   (train_phase2.py:33-45; cowreid/losses.py)
